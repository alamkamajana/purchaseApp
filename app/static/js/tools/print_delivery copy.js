...
// let offsetX = (canvas.width - imageWidth)/2 + 20;
...
// async function sendNextImageDataBatch(resolve, reject) {
...
//         const CHUNK_SIZE = 20; // Adjust based on Bluetooth LE MTU size
//             for (let i = 0; i < data.length; i += CHUNK_SIZE) {
//                 const chunk = data.slice(i, i + CHUNK_SIZE);
//                 await printCharacteristic.writeValue(chunk).then(() => {
//                     resolve();
//                 }).catch(error => reject(error));
//             }
//     } else {
...
// function sendImageData(product) {
//     index = 0;
...
function btPrintDelivery3(product) {
    ...
    navigator.bluetooth.requestDevice({ filters: [{ services: [SERVICE] }] }).then(device => {
        ...
        let chunks = splitDataIntoChunks(DATA, 128);
